...
def scan_technical_patterns(days: int = 60) -> list[dict]:
    ...
    targets: dict[str, str] = {
        "nikkei225": INDICATOR_LABELS.get("nikkei225", "nikkei225"),
        "usdjpy": INDICATOR_LABELS.get("usdjpy", "usdjpy"),
    }
    for ticker, name, _weight in JP_HEATMAP_STOCKS + US_HEATMAP_STOCKS:
        targets.setdefault(ticker, name)
    ...
    for symbol, name in targets.items():
        rows = db.get_recent_market(symbol, days=days)
        ohlc_rows = [
        ...
        if len(ohlc_rows) < 5:
            continue
        for match in patterns.detect_all(ohlc_rows):
            results.append(
                {
                    "name": name,
                    "date": ohlc_rows[-1]["date"],
...
def main() -> None:
    ...
    pattern_alerts = scan_technical_patterns()
